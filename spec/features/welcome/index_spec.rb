   ...
   describe 'When a user visits the root path "/"' do
      ...
      it "They see a list of existing users, which links to the individual user's dashboard" do
        within("#existing_users") do
          ...
          expect(page).to have_link("#{User.first.email}", href: "users/#{User.first.id}/dashboard")
          expect(page).to have_link("#{User.last.email}", href: "users/#{User.last.id}/dashboard")
        end
      end
